...
while True:
    ...
    if output == 'Select Photo':
        path = fileopenbox(msg="Please select a photo for disease detection", title="Select Photo", filetypes=["*.jpg", "*.png", "*.jpeg"])
        
        if path:
            pred = buttonbox(msg="Selected Photo", title='Detection', image=path, choices=['Proceed', 'Cancel'])
            if pred == 'Proceed':
                ...
                msgbox(msg=f"The detected class is: {output_category}", title="Result", ok_button="OK")
            else:
                continue
        else:
            msgbox(msg="No file selected. Please select a file to proceed.", title="Error", ok_button="OK")
    else:
        sys.exit()
